...
class ModelWrapper(nn.Module):
        ...
        for k, v in inputs.items():
            if type(v) == torch.Tensor:
                ...
                for k_i, v_i in v.items():
                    self._inputs[k] = {}
                    # Currently only support dict up to 2 layers
                    assert type(v_i) == torch.Tensor
                    self._keys.append([k, k_i])
                    rtn.append(v_i)
            else:
                self._inputs[k] = v
    ...
    def forward(self, inputs):
        assert len(inputs) == len(self._keys)
        for i in range(len(inputs)):
            if type(self._keys[i]) == list:
                self._inputs[self._keys[i][0]][self._keys[i][1]] = inputs[i]
            else:
                self._inputs[self._keys[i]] = inputs[i]
...
class Trainer(TrainerBase):
    def __init__(self,
        ...
        self._require_graph = require_graph
    ...
    def _train_epoch(self, epoch: int) -> Dict[str, float]:
        ...
        train_generator = lazy_groups_of(raw_train_generator, num_gpus * self._num_gradient_accumulation_steps)
        num_training_batches = math.ceil(
            self.iterator.get_num_batches(self.train_data) / (num_gpus * self._num_gradient_accumulation_steps))
