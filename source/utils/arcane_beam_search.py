...
class Arcane_BeamSearch:
    ...
    #  When debugging beam search. You should be mainly interested in the following three fields: backpointers,
    # predictions and restricted_beam_log_probs
    def search(self,
        ...
        for key, value in state.items():
            if isinstance(value, torch.Tensor):
                ...
                for value_i in value:
                    # new_value.extend([value_i] * self.beam_size)   # this is incorrect
                    # new_value.extend([deepcopy(value_i) for _ in range(self.beam_size)])
                    for _ in range(self.beam_size):
                        new_value.append(deepcopy(value_i))
                state[key] = new_value
            else:
        ...
        for timestep in range(self.max_steps - 1):
            ...
            if (last_predictions == self._end_index).all():
                break
                # TODO: if we are doing the logged sum. Perhaps we can terminate when the top-1 beam ends
            ...
            if not self.length_normalization:
                ...
                length = self.get_length(state["predictions"]).unsqueeze(-1)
    ...
    def get_length(self, predictions: torch.Tensor):
        '''
        Both @start and @end should be excluded for length
        :param predictions: shape (batch_size * beam_size, seq_len)
        :return: the length of each trajectory
        '''
        length = predictions.new_zeros(predictions.shape[0])
        for i in range(len(length)):
            indices_end = (predictions[i] == self._end_index).nonzero()
            if indices_end.shape[0] == 0:
                # There is no @end, so only need to minus 1 (i.e., exclude @start)
                length[i] = predictions.shape[1] - 1
            else:
                first_occ = indices_end.min()
                length[i] = first_occ - 1  # -1 to exlude @start
